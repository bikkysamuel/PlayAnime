...
object Constants {
    ...
    const val ERROR_MESSAGE_ON_API_FAILURE =
        "Could not load data due to an error. Please try later."
    ...
    const val WEB_VIEW_REDIRECT_DOWNLOAD_VIDEO_URL_PREFIX =
        "https://gredirect.info/download.php?url="

    const val WEB_VIEW_FULLSCREEN_RATIO = "1:0" // 1:0 is for Full Screen
    const val WEB_VIEW_NORMAL_WINDOW_RATIO =
        "16:10.59" //correct ratio for the video frame used - to avoid scroll in view and show all controls
    ...
    const val DATASTORE_DISCLAIMER_ACCEPTED = "disclaimer_accepted"

    const val DISCLAIMER = "Disclaimer"
    const val DISCLAIMER_MESSAGE = """
        PlayAnime is provided as-is, and the developers offer no guarantees regarding its functionality 
        or the availability of content. Use at your own discretion.
        
        The anime videos/data are sourced from another website not owned or affiliated with the PlayAnime 
        app or its developers. We do not claim ownership of these videos/data."""
